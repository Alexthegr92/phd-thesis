...
The implementation of AP will be discussed in this section. The following work is based on the implementation of AP used in \cite{brown2019aura}, however, with many newer additions. Full details of the implementation of AP have been included to give a complete and whole solution.
...
\begin{figure}[!t]
	...
	\end{subfigure}
	\caption{Corner topologies of server regions}
...
\begin{enumerate}
	\item A migration buffer for sending objects to the server corresponding with that boundary. Each element in the buffer is a message for one object migration. Each migration message contains all data necessary to reproduce the object on the receiving server. 
	\item An aura creation/update buffer, each element of the buffer is a message for the creation or update of an aura and includes ID, position and radius of the aura. The contents of this buffer are sent to all neighbouring servers, not just the server corresponding with this boundary. The messages structure for both creation and update are identical as the receiving server creates a new aura if one does not already exist for the corresponding object (determined using the ID), otherwise the message is just used to update the state of the existing aura. 
	\item An aura delete buffer, each element of the buffer is a message for the deletion of an aura, which consists of the aura's ID. The contents of this buffer are sent to all neighbouring servers, not just the server corresponding with this boundary.
\end{enumerate}
...
How the auras in AP are implemented will now be described at a high-level, before a formal description of the algorithms using pseudo-code are given in \ref{Algorithms}.
...
When a server begins communicating an aura, a message is added to the aura creation/update buffer. This message consists of the position of the centre of the aura and the radius of the aura. % and a flag signifying whether or not this is a creation message or not. Flag is not currently used as often causes issue in the corner case, possibly due to multiple types of messages concerncing one object being sent and recieved in the same tick.
...
Whenever an object projecting an aura moves, an update message is added to the aura creation/update buffer, containing the new position and radius of the aura (aura radius is currently fixed, but may become dynamic in the future). The contents of the buffer are broadcast to all neighbouring servers. When the message is received, the positions of the auras are updated. 
...
\begin{itemize}
	\item $AC$ - Object migration as a result of an aura collision.
	\item $BT$ - Object migration as a result of boundary traversal.
	\item $OBC$ - Aura creation, when an object collides with a server boundary.
	\item $OBU$ - Aura update, which occurs every update for all existing auras.
	\item $OBE$ - Aura destruction, when an object exits a server boundary.
	\item $BNU$ - Boundary network update, occurs every update for each network connection between servers and exchanges object migrations and aura messages for all boundaries.
\end{itemize}
...
$BT$ is called when an object traverses a boundary. In order to prevent `islands' forming (for example Objects H, I and J in Fig. \ref{AuraProj}), a recursive search is carried out to determine if an object is part of an island or not. If an object is found to not be part of an island, the entire cluster of objects is added to the send buffer, otherwise no action is taken. Note that a cluster may consist of only one object, meaning the algorithm handles migration of both single and groups of objects.
...
The implementation of AP will be discussed in this section. The following work is based on the implementation of AP used in \cite{brown2019aura}, however, with many newer additions. We include full details of the implementation of AP to give a complete and whole solution.

\section{Implementation Summary}
In this chapter the following points were discussed: 
\begin{itemize}
	\item The system architecture of AP, including how the servers are deployed in the cloud and how they communicate with a client.
	\item A breakdown of the components of each server.
	\item Topologies of server regions, including examples and the communication requirements between servers.
	\item Potential server deployments to be investigated in the future, such as the use of overseer nodes and dedicated render nodes for game streaming.
	\item A high-level description of the aura implementation including justifications for design decisions made.
	\item The algorithms used by the servers to implement AP.
	\item The life cycle of a simulated object as an FSM.
	\item The features and implementation details of the visualiser, including a description of its features and future work to be carried out on the visualiser.
\end{itemize}
