...
HLA is very similar to the model used by SpatialOS \cite{SpatialOS}, which also uses a runtime, however, the implementation details of SpatialOS's runtime are not public knowledge.

HLA is ideal for heterogeneous distributed simulations and for re-use with different types of simulations. However, there are some disadvantges to this architecture:
\begin{enumerate}
...
In the case of real-time physics, we can assume the use of either homogeneous simulations or at least simulations that will be simulating physics with a large degree of shared semantics between simulations (as all simulations are dealing with physical simulation). As a result of this, there is little to no advantage gained in using the HLA architecture and direct communication between servers has the benefit of lower latency between servers.
